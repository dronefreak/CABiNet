"""Unit tests for src/datasets/aeroscapes.py
...
Tests the pre-converted images/+masks/ dataset loader:
  ...
  - DataLoader integration with default collation, including a batch_size>1
    val-mode batch (AeroScapes source images are uniformly 1280x720, unlike
...
class TestAeroScapesInitErrors:
    def test_invalid_mode_raises(self, tmp_path):
        _make_dataset_root(tmp_path, "train", {"000001_001": 1})
        with pytest.raises(ValueError, match="not supported"):
            AeroScapes(
                ignore_lb=255, rootpth=str(tmp_path), cropsize=(4, 4), mode="bogus"
            )

    def test_missing_root_raises(self):
        with pytest.raises(FileNotFoundError, match="does not exist"):
            AeroScapes(ignore_lb=255, rootpth="/nonexistent/path", cropsize=(4, 4))
    ...
    def test_no_valid_pairs_raises(self, tmp_path):
        ...
        Image.fromarray(np.zeros((4, 4, 3), dtype=np.uint8)).save(
            tmp_path / "images" / "train" / "000001_001.jpg"
        )
...
class TestFileDiscovery:
    def test_finds_all_matching_pairs(self, tmp_path):
        _make_dataset_root(
            tmp_path,
            "train",
            {"000001_001": 0, "000001_002": 1, "000002_001": 2},
        )
        ds = AeroScapes(
            ignore_lb=255, rootpth=str(tmp_path), cropsize=(4, 4), mode="train"
        )
    ...
    def test_image_with_no_matching_mask_is_skipped_with_warning(self, tmp_path):
        _make_dataset_root(tmp_path, "train", {"000001_001": 0})
        # Add an orphan image with no mask counterpart
        Image.fromarray(np.zeros((4, 4, 3), dtype=np.uint8)).save(
            tmp_path / "images" / "train" / "000001_002.jpg"
        )
    ...
    def test_train_val_test_are_independent(self, tmp_path):
        ...
        assert len(AeroScapes(255, str(tmp_path), (4, 4), mode="train")) == 2
        assert len(AeroScapes(255, str(tmp_path), (4, 4), mode="val")) == 1
        assert len(AeroScapes(255, str(tmp_path), (4, 4), mode="test")) == 3
...
class TestGetItem:
    def test_returns_two_tuple(self, tmp_path):
        ...
        ds = AeroScapes(
            ignore_lb=255, rootpth=str(tmp_path), cropsize=(4, 4), mode="val"
        )
        item = ds[0]
    ...
    def test_mask_pixel_values_pass_through_untouched(self, tmp_path):
        """Pixel value 6 (a real class ID) and 255 (ignore) must survive
        exactly — masks are already final class IDs, no LUT translation."""
    ...
    def test_train_mode_output_matches_cropsize(self, tmp_path):
        """RandomCrop must fire on the full image — output spatial size must
        equal cropsize regardless of source size.
        ...
        size — harmless in production (real cropsize is 640x640) but not
        exercised by a smaller test cropsize.
        """
    ...
    def test_train_mode_handles_source_smaller_than_cropsize(self, tmp_path):
        ...
        than cropsize without erroring."""
...
class TestDataLoaderIntegration:
    def test_default_collate_produces_correct_batch_shape(self, tmp_path):
        _make_dataset_root(
            tmp_path,
            "train",
            {f"000001_{i:03d}": 0 for i in range(6)},
            size=(128, 128),
        )
        ds = AeroScapes(
            ignore_lb=255, rootpth=str(tmp_path), cropsize=(64, 64), mode="train"
    ...
    def test_val_batch_size_greater_than_one_works_for_uniform_resolution(
        ...
        AeroScapes source images are uniformly 1280x720, so a batched val
...
class TestAugmentationConfig:
    def test_default_augmentation_used_when_none_passed(self, tmp_path):
        ...
        ds = AeroScapes(
            ignore_lb=255, rootpth=str(tmp_path), cropsize=(8, 8), mode="train"
        )
...
class TestMixUp:
    ...
    def test_mixup_probability_one_always_blends(self, tmp_path, monkeypatch):
        _make_dataset_root(
            tmp_path, "train", {"a": 1, "b": 2, "c": 3}, size=(128, 128)
        )
        ds = AeroScapes(
            ignore_lb=255,
            rootpth=str(tmp_path),
    ...
    def test_mixup_blends_image_but_label_stays_hard(self, tmp_path):
        _make_dataset_root(tmp_path, "train", {"a": 1, "b": 9}, size=(128, 128))
        ds = AeroScapes(
            ignore_lb=255,
            rootpth=str(tmp_path),
            cropsize=(64, 64),
