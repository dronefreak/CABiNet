...
class ConvBNReLU(nn.Module):
    def __init__(
        self, in_chan, out_chan, kernel_size=3, stride=1, padding=1, *args, **kwargs
    ):
...
class AttentionBranch(nn.Module):
    def __init__(self, inplanes, interplanes, outplanes, num_classes):
        ...
        self.convb = nn.Conv2d(
            interplanes, outplanes, kernel_size=1, stride=1, padding=0, bias=True
        )

        self.b1 = nn.Conv2d(
            inplanes + interplanes, outplanes, kernel_size=3, padding=1, bias=False
        )
...
class CABiNet(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_ab = F.interpolate(
            feat_ab, (feat_sb.size()[2:]), mode="bilinear", align_corners=True
        )
        feat_ab_final = F.interpolate(
            feat_ab_final, (feat_sb.size()[2:]), mode="bilinear", align_corners=True
        )
        ...
        feat_con = F.interpolate(
            feat_ab_final, (H, W), mode="bilinear", align_corners=True
        )
