...
# AeroScapes is also UAV-captured imagery, so the same augmentation recipe
# applies unchanged. See uavid.py's module docstring for the rationale
# behind each knob (mosaic/copy_paste unsupported, mixup's hard-label
# simplification, etc.) — identical here.
...
class AeroScapes(Dataset):
    ...
    ``src/scripts/convert_aeroscapes_to_yolo.py``::
        ...
        <rootpth>/
        ├── images/
        │   ├── train/   ← RGB JPEGs (uniformly 1280x720)
        │   └── val/
        └── masks/
            ├── train/   ← single-channel PNGs, pixel value = class ID
            └── val/

    Mask pixel values are already final trainIds (0-11; Background=0 …
    Sky=11 — see configs/AeroScapes_info.json) — no RGB colour palette or
    lookup table is needed here, unlike UAVid's raw distribution. Pixel
    value 255 is reserved for genuinely unrecognized values, not a real
    class. There is no source test split (see the conversion script's
    docstring) — only ``"train"``/``"val"`` are populated by the converter,
    though ``mode="test"`` is still accepted here for API parity with
    :class:`~src.datasets.uavid.UAVid` / :class:`~src.datasets.cityscapes.CityScapes`.
    ...
    rootpth:
        Root of the *converted* dataset (i.e. ``convert_aeroscapes_to_yolo.py``'s
        ``--dst``) — NOT the raw AeroScapes distribution.
    cropsize:
        ``(H, W)`` crop applied during training augmentation via
        ``RandomCrop``. AeroScapes source images are uniformly 1280x720
        (unlike UAVid's mixed resolutions), so ``RandomCrop`` mainly serves
        as a fixed-size training patch here rather than a resolution
        normalizer — ``pad_if_needed=True`` is kept for robustness.
    mode:
        ``"train"``, ``"val"``, or ``"test"``.
    augmentation:
    ...
    ----------------------------
    Unlike UAVid, AeroScapes source images are uniformly 1280x720, so
    ``val``/``test`` mode (no crop applied) can still be batched with
    ``batch_size > 1`` via the default collate function — no special-casing
    is needed in train.py/evaluate.py for this dataset.
    """
    ...
    def __init__(
        ...
        # --- Image normalisation --------------------------------------------------
        # Mean/std computed from the AeroScapes train set (see
        # src/datasets/compute_aeroscapes_stats.py) — distinct from ImageNet
        # and UAVid stats.
        self.to_tensor = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Normalize(
                    mean=(0.439, 0.508, 0.460),
                    std=(0.176, 0.157, 0.194),
                ),
            ]
        )
        ...
        print(
            f"[INFO] AeroScapes dataset loaded: {self.len} samples ({mode}) from {img_dir}"
        )
...
# ---------------------------------------------------------------------------
# Smoke test (run: python src/datasets/aeroscapes.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    import sys

    rootpth = os.environ.get("AEROSCAPES_YOLO_ROOT", "")
    if not rootpth:
        print(
            "Set AEROSCAPES_YOLO_ROOT=/path/to/converted/aeroscapes_yolo and re-run "
            "(this must be the OUTPUT of convert_aeroscapes_to_yolo.py, not raw AeroScapes data)."
        )
        sys.exit(0)
